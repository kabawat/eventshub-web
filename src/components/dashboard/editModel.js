...
import { updateEventsService } from '@/lib/services/admin'
...
const EditModel = ({ data, handleClose }) => {
    ...
    const handleUpdate = async () => {
        try {
            ...
            const res = await updateEventsService(payload, formData?._id)
            toast(res?.message)
        ...
        <Modal show={true} centered animation={true} size='lg' >
            <Modal.Header className='d-flex justify-content-between align-items-center'>
                ...
                <div className="form-check form-switch">
                    ...
                    <input className="form-check-input" type="checkbox" role="switch" name="isActive" onChange={handleChange} value={`${formData?.isActive || false}`} checked={formData?.isActive || false} />
                </div>
            </Modal.Header>
            <Modal.Body>
                ...
                </Button>
                {
                    loader ? <>
                        <Button variant="warning" className='text-light' disabled>
                            Save Changes <span className="spinner-border spinner-border-sm  ms-2 text-light" role="status"></span>
                        </Button>
                    </> : <>
                        <Button variant="warning" className='text-light' onClick={handleUpdate}>
                            Save Changes
                        </Button>
                    </>
                }
